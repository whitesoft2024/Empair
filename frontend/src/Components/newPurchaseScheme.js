...
function NewPurchaseScheme({ show, handleClose }) {
    ...
    const fetchSchmeName = async () => {
        try {
            const response = await fetch('http://localhost:4000/api/getAllScheme');
            if (!response.ok) {
    ...
    if (
        ...
        try {
            ...
            const response = await axios.post('http://localhost:4000/api/addCustomers', updatedFormData);
    ...
    const fetchReference = async (searchTerm) => {
        try {
            const response = await fetch(`http://localhost:4000/api/getReference?mobileNumber=${searchTerm}`);
            if (!response.ok) {
